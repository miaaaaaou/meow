...
#  🀄 ⬜📜 + 🪶 `trailer` (`Key: value` , `git` 📐) : `*-by:` · `Claude-Session:` · `Change-Id:`
#       + 🤖📎 `Generated by Claude Code` · 🔗 · `#🔢`/`(#🔢)` · 🐍🔑/🏦 · 📁 · `…` · 🔠   →  🚿🙈
#  🐈✍️ (🔏 `signature` ∨ 📧 `claude`/`noreply`) 🗣️🔤/🈲 → 😾❌ (exit 1)
#  🙋✍️ (👤)                         🗣️🔤/🈲 → 🗒️⚠️ ☝️ (🚫🙅 : 🙋 🙇✅)
#  🕰️📜 🍂 = 🍂 (🚫 ✍️🕰️ , 🚫🔨 `meow`)   ·   🪞 `.claude/𓊗𓆓.py` 📐 (𓉗𓏤 ♻️)
#  🚨 #86 : 🔀💬 (`squash`) 🪶 `trailer` 🤖✍️📎 → 🚿 ∀ 𓊕 (`*-by:` 📐 , 🚫 ☝️ 3️⃣🔒) → 🚫 `meow` 🧊

#  🤖📎🧱 + 🔢🔗  —  🚿 ☝️  ( ⛙📨 : 🀄 🐍💬📄 🙈 `𓅗𓎜` ♻️ + 🤖📎 )
#  🪶 `trailer` 𓊕 : `<x>-by:` (∀ `co-authored`/`signed-off`/`reviewed`/…) ∨ `session`/`id` → 🀫🙈
𓅘𓊫 = [
    re.compile(r"(?im)^[ \t]*(?:[a-z]+(?:-[a-z]+)*-by|claude-session|"
               r"change-id|message-id|generated-by):.*$"),
    re.compile(r"(?i)generated by claude code"),     # 🤖📎 `markdown` 🖋️
    re.compile(r"\(#\d+\)|#\d+"),                     # 🔢 🔗
]
