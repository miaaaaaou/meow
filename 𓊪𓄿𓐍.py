...
def 𓊪𓁐𓎗():
    ...
    assert "🙀" in 𓊾                     # 💥 🀫 ✏️ (🐈∩🐕)
    𓋀 = 𓊾.split("\n")
    assert len(𓋀) == 𓅓.𓈖𓏏              # 🗺️ 📐 🗿 (🙀 = 1️⃣🀫)
    # ↩️ 🀄 : 🐕 ≠ 🐈 → 🙀 ∅ , 🐕 ✏️
    𓋂 = 𓅓.𓉔(random.Random(0))
    assert "🙀" not in 𓋂.𓁐() and "🐕" in 𓋂.𓁐()
...
def 𓊪𓊆𓂺𓅘():
    # 🩹 #90 : `No` 🔢 (`²` `③` `⑦` `₇`) — `isdigit` ✅ ⚔️ `int` 💥 → 🛡️ 🏁🏁 1 , 🚫💥
    for 𓅕 in ("²", "③", "⑦", "₇"):
        assert 𓅓.𓊆𓂺([𓅕]) == 1             # 🚫💥 ‼️  (⏮️🩹 → ValueError 🚀💥)
    assert 𓅓.𓊆𓂺(["⑦", "3️⃣"]) == 3   # `No` 🚮 → ⏭️ 👀 1️⃣..9️⃣ 🀄 ✅
    assert 𓅓.𓊆𓂺(["\u0667"]) == 7     # `Nd` 🌐 → `int` ✅ 🗿  (↔️ 🚫∆)
    assert 𓅓.𓊆𓂺(["\U0001d7d9"]) == 1      # `Nd` 🌐 → ✅ 🗿
    𓂺 = subprocess.run(
        [sys.executable, "-m", "𓃠𓐍𓅓", "🤖", "²"],
        capture_output=True, text=True, timeout=60,
    )
    assert 𓂺.returncode == 0               # 🚀 ✅ , 🚫 🐍💥📜
    assert "ValueError" not in 𓂺.stderr
...
def 𓊪𓅘():
    ...
    assert 𓆧.𓅘(𓊐) == ([], []), 𓆧.𓅘(𓊐)
    # 🚨 #86 : 🔀💬🎭 (`squash`) + 🪶 `trailer` 🀫 → ∅  ( ∀ `*-by:` , 🚫 ☝️ 3️⃣🔒 )
    #   `Signed-off-by:` · `Reviewed-by:` · `Acked-by:` · `Change-Id:` · `Message-Id:`
    𓊑 = ("😻 #86 (#85)\n\n"
          "Signed-off-by: Cat <c@x>\nReviewed-by: Tabby <t@x>\n"
          "Acked-by: Kitten <k@x>\nChange-Id: I0badcafe\nMessage-Id: <m@x>\n"
          "Co-authored-by: Claude <x>\nClaude-Session: https://x")
    assert 𓆧.𓅘(𓊑) == ([], []), 𓆧.𓅘(𓊑)
    # 🗣️🔤🌿💬 (🪶 `trailer` 🚿 ⏳ 🥩 🔤 🎯) → 💉  ( 🚫 🀫🙈 🥩 )
    𓊒, _ = 𓆧.𓅘("🐈 refactor guard\n\nSigned-off-by: Cat <c@x>")
    assert set(𓊒) == {"refactor", "guard"}, 𓊒
    # 🔠 (⏫🔤) + 🐍🔑 + `#🔢`/`(#🔢)` → ∅
...
def 𓊪𓋝():
    ...
    #        🐕(10,0) 🔭 (📏>𓊄𓎿) → 🚫 💨 🚫 🎾 → 📏🐕 🧢 🟰 ∀ → 🐾 🏆 (👴)
    def 𓋅():
        𓋆 = 𓅓.𓉔(random.Random(0), 𓊵𓈖=0, 𓃥𓁋=True,
                  𓅱𓁋=False, 𓎛𓁋=False, 𓋔𓁋=False)
        𓋆.𓊵 = {(1, 0), (0, 1)}              # 🧱 → 🐭 🔒 → 🚫🛣️
        𓋆.𓃠 = (3, 3)
        𓋆.𓁉𓂋, 𓋆.𓊚𓂋 = [(0, 0)], [0]        # 🐭 🚫🛣️ → 📏 ≡ ∞ ∀
        𓋆.𓃥 = (10, 0)                        # 🐕 🔭 (📏 > 𓊄𓎿 → 🚫 💨)
        𓋆.𓇬, 𓋆.𓆛, 𓋆.𓊮 = (10, 7), (0, 7), (9, 6)
        𓋆.𓋯 = True                           # 🧶 🏁 → 🙈 🤾
        return 𓋆
    𓅔 = 𓅓.𓊄(𓋅())
    assert 𓅔 != "🐾", f"🙀 💤 @ 🐕 → 💀 🪟 ({𓅔})"   # 🩹 : 🏃 🀫 , 🚫 🗿
...
def 𓊪𓆵𓋝():
    # 💀0️⃣ 🔒 #110 :  🌊8-9 × 3🗺️ × 60🎲 → 🚫 💀  ( 🩸⏬ 🐈 🪤 @ 🐕❄️ 🩹 )
    #   ⏮️ : 🐈 🩸⏬ + 🪤 → 🐾 🗿 → 🐕 ❄️(#72) 🌡️ → 💀 ; 🩹 → 🏃 🀫 → 🚫 ⚰️
    for 𓊍 in (8, 9):
        for 𓉏 in (None, *𓅓.𓆵𓊗):
            for 𓊃 in range(60):
                𓋁 = 𓅓.𓊆(𓊍, random.Random(1000 + 𓊃), 𓆵𓉏=𓉏)
                for _ in range(600):
                    if 𓋁.𓂷(𓅓.𓊄(𓋁)):
                        break
                assert not 𓋁.𓋺, f"🙀 💀 ↩️ 🌊{𓊍} {𓉏} 🎲{1000 + 𓊃}"
...
𓐩 = [𓊪𓎘, 𓊪𓐍, 𓊪𓎉, 𓊪𓎗, 𓊪𓊵, 𓊪𓎘𓁉, 𓊪𓂷, 𓊪𓇬, 𓊪𓆛, 𓊪𓊙, 𓊪𓄊, 𓊪𓁐, 𓊪𓁐𓎗, 𓊪𓋴, 𓊪𓊮, 𓊪𓊰,
     𓊪𓃥, 𓊪𓃥𓎗, 𓊪𓊟, 𓊪𓃥𓎿, 𓊪𓁋,
     𓊪𓅱, 𓊪𓅱𓎗, 𓊪𓅱𓎗𓊵, 𓊪𓅲, 𓊪𓅱𓁋,
     𓊪𓎛, 𓊪𓎛𓁋,
